...
const char * const BITCOIN_CONF_FILENAME = "vishai.conf";
const char * const BITCOIN_PID_FILENAME = "vishaid.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "vishai" is a composite category enabling all Vishai-related debug output
            if(ptrCategory->count(string("vishai"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VishaiCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VishaiCore
    // Mac: ~/Library/Application Support/VishaiCore
    // Unix: ~/.vishaicore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VishaiCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/VishaiCore";
#else
    // Unix
    return pathRet / ".vishaicore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty vishai.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override vishai.conf
